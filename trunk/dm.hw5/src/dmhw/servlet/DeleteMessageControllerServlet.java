 ...
 */
 public class DeleteMessageControllerServlet extends ControllerServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (message == null) {
			this.responseMessage(request, response, "Message not exists", "Specified message already not exists", "return", Pages.index);
		}
		if (message.getRank() > user.getRank()) {
			this.responseMessage(request, response, "No permissions", "You don't have permissions to remove this message", "return", Pages.index);
		}
		MessageManager.deleteMessage(msgid);
		this.responseMessage(request, response, "Deleted successfully", "The message was deleted successfully", "return", Pages.index);
	}   	  	    
}